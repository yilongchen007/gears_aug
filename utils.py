...
def uncertainty_loss_fct(pred, logvar, y, perts, reg = 0.1, ctrl = None,
    ...
    for p in set(perts):
        if p!= 'ctrl':
            pred_p = pred[np.where(perts==p)[0]][:, mask]
            y_p = y[np.where(perts==p)[0]][:, mask]
            logvar_p = logvar[np.where(perts==p)[0]][:, mask]
        else:
        ...
        if p!= 'ctrl':
            losses += torch.sum(direction_lambda *
                                (torch.sign(y_p - ctrl[mask]) -
                                 torch.sign(pred_p - ctrl[mask]))**2)/\
