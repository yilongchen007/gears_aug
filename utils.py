...
def get_per_gene_thresholds(dataset, num_genes, num_bins=10, method="quantile"):
    """
    Computes per-gene thresholds for discretization when each data.x is [batch_size * num_genes, 1].

    Parameters:
    ----------
    dataset : list of PyG Data objects
        Each data.x is assumed to be [batch_size * num_genes, 1]
    num_genes : int
        Total number of genes (required to reshape x into [batch_size, num_genes])
    num_bins : int
        Number of bins (will return num_bins - 1 thresholds per gene)
    method : str
        "quantile" for equal-frequency bins or "uniform" for equal-width

    Returns:
    --------
    thresholds : torch.Tensor
        Shape: [num_bins - 1, num_genes]
    """
    # Step 1: reshape each x to [batch_size, num_genes]
    all_x = []
    for data in dataset:
        x = data.x.view(-1, num_genes).cpu()  # shape: [batch_size, num_genes]
        all_x.append(x)
    
    all_x = torch.cat(all_x, dim=0)  # shape: [total_samples, num_genes]

    # Step 2: compute thresholds
    if method == "quantile":
        percentiles = np.linspace(0, 100, num_bins + 1)[1:-1]  # exclude 0% and 100%
        thresholds = np.percentile(all_x.numpy(), percentiles, axis=0)  # shape: [num_bins - 1, num_genes]
        thresholds = torch.tensor(thresholds, dtype=torch.float)
    elif method == "uniform":
        min_vals = all_x.min(dim=0).values  # shape: [num_genes]
        max_vals = all_x.max(dim=0).values
        thresholds = torch.stack([
            torch.linspace(min_vals[i], max_vals[i], steps=num_bins + 1)[1:-1]
            for i in range(all_x.shape[1])
        ], dim=1)  # shape: [num_bins - 1, num_genes]
    else:
        raise ValueError("Unknown method:", method)

    return thresholds  # shape: [num_bins - 1, num_genes]
...
def get_coexpression_network_from_train(adata, threshold, k, data_path,
    ...
    if os.path.exists(fname):
        ...
        X_tr = X[np.isin(adata.obs['perturbation'], [i for i in train_perts if 'control' in i])]
        gene_list = adata.var['gene_name'].values

        if issparse(X_tr):  # Check if X_tr is a sparse matrix
            X_tr = X_tr.toarray()
...
def uncertainty_loss_fct(pred, logvar, y, perts, reg = 0.1, ctrl = None,
                         direction_lambda = 1e-3, mask = None):
    """
    ...
    Args:
        ...
        mask (dict): binary mask of observations
    ...
    for p in set(perts):
        ...
        losses += torch.sum((pred_p - y_p)**(2 + gamma) * m + reg * torch.exp(
            -logvar_p)  * (pred_p - y_p)**(2 + gamma) * m) / torch.sum(m)
                         
        # direction loss                 
        if p!= 'ctrl':
            losses += torch.sum(direction_lambda *
                                (torch.sign(y_p - ctrl) -
                                 torch.sign(pred_p - ctrl))**2 * m) / torch.sum(m)
        else:
            losses += torch.sum(direction_lambda *
                                (torch.sign(y_p - ctrl) -
                                 torch.sign(pred_p - ctrl))**2 * m) / torch.sum(m)
...
def loss_fct(pred, y, perts, ctrl = None, direction_lambda = 1e-3, mask = None):
    """
    ...
    Args:
        ...
        ctrl (str): control perturbation
        direction_lambda (float): direction loss weight hyperparameter
        mask (torch.tensor): dictionary of perturbations to conditions
    ...
    for p in set(perts):
        ...
        if p!= 'ctrl':
            ...
            pred_p = pred[pert_idx]
            y_p = y[pert_idx]
        losses = losses + torch.sum((pred_p - y_p)**(2 + gamma) * m) / torch.sum(m)
...
def get_mean_control(adata):
    ...
    """
    mean_ctrl_exp = adata[adata.obs['perturbation'] == 'control'].to_df().mean()
    return mean_ctrl_exp


def get_genes_from_perts(perts):
    """
    Returns list of unique genes involved in a given perturbation list.
    """
    if isinstance(perts, str):
        perts = [perts]
    return list(np.unique([g for p in np.unique(perts) for g in p.split('+') if g != 'control'])) 
