...
class PertData:
    ...
    def create_cell_graph(self, X, y, de_idx, pert, pert_idx,
        ...
        embed_cell = torch.Tensor(embed_cell).T
    ...
    def create_dataset_file(self):
        ...
        for split in tqdm(self.adata.obs['split'].unique(), desc="Splits"):
            self.dataset_processed[split] = []
            for p in tqdm(self.adata.obs['perturbation'].unique(), desc=f"Perturbations in {split}", leave=False):
                self.dataset_processed[split].extend(self.create_cell_graph_dataset(self.adata[self.adata.obs['split']==split], p))
        print_sys("Done!")
